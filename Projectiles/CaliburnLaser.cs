...
{
    public class CaliburnLaser : ModProjectile
    {
		...
		public override void AI()
		{
			...
			{
				Main.PlaySound(new LegacySoundStyle(2, 15), player.position);
				projectile.ai[1] = 0;
			}
		...
		public Color color = new Color(251, 250, 62);
		private void SpawnDusts(Player player)
		{
			...
			if (Main.rand.NextBool(5))
			{
				...
				Dust dust = Main.dust[Dust.NewDust(dustPos + offset - Vector2.One * 4f, 8, 8, 31, 0.0f, 0.0f, 100, color, 1.5f)];
				dust.velocity *= 0.5f;
				dust.velocity.Y = -Math.Abs(dust.velocity.Y);
				unit = dustPos - Main.player[projectile.owner].Center;
				unit.Normalize();
				dust = Main.dust[Dust.NewDust(Main.player[projectile.owner].Center + 55 * unit, 8, 8, 31, 0.0f, 0.0f, 100, color, 1.5f)];
		...
		private void ChargeLaser(Player player)
		{
			...
			else
			{
				...
				for (int k = 0; k < chargeFact + 1; k++)
				{
					...
					Dust dust = Main.dust[Dust.NewDust(pos, 20, 20, 31, projectile.velocity.X / 2f, projectile.velocity.Y / 2f, 0, color)];
